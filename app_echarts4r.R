    ...
    f7Tabs(animated = TRUE,
           ...
           f7Tab(
             ...
             f7Card(title = "Jumlah Kasus Harian",
                    echarts4rOutput("plotharian", width = "100%")
                    )
             ), 
           f7Tab(
             ...
             f7Card(title = "Tren Kasus Provinsi",
                    echarts4rOutput("trenprov", width = "100%")
             )
             ), 
           f7Tab(
             ...
             f7Card(
               ...
               echarts4rOutput("trennasional", width = "100%")
               )
             ,
             f7Card(
               title = "Tren Rasio Sembuh vs Meninggal",
               echarts4rOutput("trenratio", width = "100%")
               )
             )
           )
  )
...
server <- function(input, output, session){
  ...
  output$trennasional <- renderEcharts4r({
    tren() %>% 
      group_by(Kasus) %>% 
      e_charts(x = Tanggal) %>% 
      e_datazoom(
      ...
      e_tooltip() %>% 
      # e_title("Tren Kasus Nasional", "") %>%
      e_x_axis(Tanggal, axisPointer = list(show = TRUE)) %>% 
      e_line(value) %>% 
      e_grid(left = 50) %>% 
      e_show_loading()
  ...
  output$trenratio <- renderEcharts4r({
    ...
    ratio() %>% 
      ...
      e_tooltip() %>% 
      # e_title("Tren Rasio Sembuh vs Meninggal", "") %>%
      e_x_axis(Tanggal, axisPointer = list(show = TRUE)) %>% 
      e_line(value) %>% 
      e_grid(left = 50) %>% 
      e_show_loading()
  ...
  output$plotharian <- renderEcharts4r({
  ...
  output$trenprov <- renderEcharts4r({
    trendprov %>% 
      filter(Provinsi == input$provnm) %>%
